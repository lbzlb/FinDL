...
# ===== 配置区域（修改此处） =====
# 模型目录
MODEL_DIR = "/data/project_20251211/experiments/timexer_v0.45_20260323185436_20260214123004"
...
# 预测数据文件（.pt文件路径，用于阶段二）
PREDATA_FILE = "/data/project_20251211/data/pre_data/processed_data_20260308_v0.01/data.pt"
...
def compute_company_statistics(inference_results: list) -> pd.DataFrame:
    ...
    company_stats_df = company_stats_df.sort_values('company_id', key=lambda x: pd.to_numeric(x, errors='coerce')).reset_index(drop=True)
...
def save_prediction_report(prediction_df: pd.DataFrame, company_stats_df: pd.DataFrame, output_dir: Path):
    ...
    if 'company_id' in sheet1_df.columns:
        sheet1_df = sheet1_df.sort_values('company_id', key=lambda x: pd.to_numeric(x, errors='coerce')).reset_index(drop=True)
    sheet1_df.columns = [
    ...
    if 'company_id' in sheet2_df.columns:
        sheet2_df = sheet2_df.sort_values('company_id', key=lambda x: pd.to_numeric(x, errors='coerce')).reset_index(drop=True)
    sheet2_df.columns = [
    ...
    if 'company_id' in sheet3_df.columns:
        sheet3_df = sheet3_df.sort_values('company_id', key=lambda x: pd.to_numeric(x, errors='coerce')).reset_index(drop=True)
    ...
    merged = prediction_df_copy.merge(
        ...
        on='_merge_key', how='left', suffixes=('', '_stats')
    )
    # 清理merge辅助列
    ...
    merged = merged.sort_values('company_id', key=lambda x: pd.to_numeric(x, errors='coerce')).reset_index(drop=True)
...
def save_company_ranking(company_stats_df: pd.DataFrame, output_dir: Path):
    ...
    for sheet_name, value_col in reference_configs:
        ...
        # 按company_id数字大小排序
        ref_df = ref_df.sort_values('company_id', key=lambda x: pd.to_numeric(x, errors='coerce')).reset_index(drop=True)
...
def save_historical_summary(company_stats_df: pd.DataFrame, output_dir: Path):
    ...
    output_stats_df = output_stats_df.sort_values('company_id', key=lambda x: pd.to_numeric(x, errors='coerce')).reset_index(drop=True)
...
def save_company_details(inference_results: list, company_stats_df: pd.DataFrame, output_dir: Path):
    ...
    results_df = results_df.sort_values('company_id', key=lambda x: pd.to_numeric(x, errors='coerce')).reset_index(drop=True)
    company_groups = results_df.groupby(['company_id', 'company_name', 'stock_code'], sort=False)
